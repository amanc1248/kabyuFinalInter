...
class CategoryBooks extends StatefulWidget {
  ...
  const CategoryBooks({Key key, this.book}) : super(key: key);
...
class _CategoryBooksState extends State<CategoryBooks> {
  checkBookImage() {
    if (widget.book.image == null) {
      return Container(
          ...
          child: Center(
            child: Text(
              '${widget.book.title[0]}'.toUpperCase(),
  ...
  Widget build(BuildContext context) {
    ...
    // print(widget.book.image);
    return GestureDetector(
      onTap: () {
        // productProvider.bookIndexForChat =widget.book.bo;
        productProvider.currentProduct = widget.book;
        Navigator.push(
            context,
            MaterialPageRoute(
